...
function ready() {
    ...
    var btnWatchMovie = document.getElementsByClassName("watch-movie");
    ...
    for (var i = 0; i < btnWatchMovie.length; i++) {
        var history = btnWatchMovie[i];
        history.addEventListener("click", addHistory);
    }
...
function addHistory(event) {
    var button = event.target;
    var movie = button.parentElement.parentElement.parentElement;
    var titleHistory = movie.getElementsByClassName("title")[0].innerText;
    var imageHistory = movie.getElementsByClassName("img-playlist")[0].src;
    addToHistory(titleHistory, imageHistory);
}

function addToHistory(titleHistory, imageHistory) {
    var contentHistory = document.createElement("div");
    contentHistory.classList.add("content-history");
    var filmHistory = document.getElementsByClassName("film-history")[0];
    var toGetAlert = filmHistory.getElementsByClassName("movie-name");
    for (var i = 0; i < toGetAlert.length; i++) {
        if (toGetAlert[i].innerText === titleHistory) {
            contentHistory.parentElement.remove();
        }
    }
    var newBoxHistory = `<img src="${imageHistory}" alt="">
                        <i class="fa-solid fa-trash"></i>
                        <div class="movie-name">${titleHistory}</div>`;
    contentHistory.innerHTML = newBoxHistory;
    filmHistory.append(contentHistory);
    contentHistory.getElementsByClassName("fa-trash")[0].addEventListener("click", deleteBox);
}